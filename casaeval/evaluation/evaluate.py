...
def aa_match_prefix(
    peptide1: List[str], # True Sequences
    peptide2: List[str], # Casanovo Predictions
    ...
    match_scores = np.zeros(max(len(peptide1), len(peptide2)), np.float64)
    ...
    while i1 < len(peptide1) and i2 < len(peptide2):
        ...
        if (
            ...
            #if aa_matches[max(i1, i2)]:
            match_scores[max(i1, i2)] = aa_scores_list[i2]
            i1, i2 = i1 + 1, i2 + 1
    ...
    return aa_matches, aa_matches.all(), match_scores
...
def aa_match_prefix_suffix(
    ...
    """
    # Find longest mass-matching prefix.
    aa_matches, pep_match, match_scores = aa_match_prefix(
        peptide1, peptide2, aa_dict, aa_scores_list, cum_mass_threshold, ind_mass_threshold
    )
    # No need to evaluate the suffixes if the sequences already fully match.
    if pep_match:
        return aa_matches, pep_match, match_scores
...
def aa_match(
    ...
    peptide2: List[str],
    aa_dict: Dict[str, float],
    aa_scores_list, # Modification: added aminoacid scores for better retrieval!
    cum_mass_threshold: float = 0.5,
    ind_mass_threshold: float = 0.1,
    mode: str = "best",
) -> Tuple[np.ndarray, bool]:
    ...
    if mode == "best":
        ...
        return aa_matches[::-1], pep_match, match_scores
    else:
...
def aa_match_batch(
    ...
    peptides2: Iterable,
    aa_dict: Dict[str, float],
    aa_scores_list, # Modification: Added aa_score_list for better retrieval
    cum_mass_threshold: float = 0.5,
    ind_mass_threshold: float = 0.1,
    mode: str = "best",
...
def aa_precision_recall(
    ...
    #### for Debugging
#peptides1 = [['A', 'R', 'N', 'D', 'N', 'E'], ['A', 'R', 'N', 'D'], ['A', 'R', 'N', 'D']]
#peptides2 = [['A', 'R', 'N', 'E'], ['A', 'E', 'E', 'N', 'D'], ['A', 'R', 'N', 'D']]
#aa_dict = {'A': 71.03711, 'R': 156.10111, 'N': 114.04293, 'D': 115.02694, 'E': 129.04259}
#aa_score_list = [[0.5, 0.6, 0.7, 0.8], [0.5, 0.6, 0.7, 0.8, 0.9], [0.5, 0.6, 0.7, 0.8]]
#cum_mass_threshold = 0.5
#ind_mass_threshold = 0.1
#mode = "best"

#result = aa_match_batch(peptides1, peptides2, aa_dict, aa_score_list, cum_mass_threshold, ind_mass_threshold, mode)
#print(result)